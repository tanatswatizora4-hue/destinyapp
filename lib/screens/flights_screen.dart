...
enum _FlightUiPhase { form, results, inbound, selected }
...
class _FlightsScreenState extends State<FlightsScreen> {
  ...
  Future<void> _selectOffer(FlightOffer offer) async {
    ...
    try {
      ...
      setState(() {
        _loading = false;
        _error = 'Unable to continue with this itinerary.';
      });
    }
  }
  ...
  Future<void> _selectInbound(FlightOffer offer) async {
    ...
    setState(() => _inboundOffer = offer);
    await _validate([_outbound!, offer]);
  }
  ...
  Future<void> _validate(List<FlightOffer> selections) async {
    ...
    try {
      final displayed = selections.last.totalPrice;
      ...
      setState(() {
        ...
        _phase = _FlightUiPhase.selected;
        _loading = false;
  ...
  Future<void> _continueEnquiry() async {
    ...
    if (query == null || offer == null) return;
    if (!_signedIn) {
  ...
  Widget _buildResults() {
    ...
    if (_phase == _FlightUiPhase.selected && _validated != null) {
      return _selectedPanel(_validated!);
    }

    final list = _phase == _FlightUiPhase.inbound
        ? _inbound?.offers ?? const <FlightOffer>[]
        ...
        ...list.map(
          (offer) => Padding(
            ...
            child: FlightOfferCard(
              ...
              onTap: () {
                if (_phase == _FlightUiPhase.inbound) {
  ...
  Widget _selectedPanel(FlightOfferValidation validation) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          FilledButton(
            onPressed: _loading ? null : _continueEnquiry,
            style: FilledButton.styleFrom(
          ...
          TextButton(
            ...
            child: const Text('Choose another itinerary'),
          ),
        ],
      ),
    );
